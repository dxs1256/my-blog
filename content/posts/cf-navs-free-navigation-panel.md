...
categories:
  - 工具推荐
image: "https://bing.ee123.net/img/rand?seed=cf-navs-free-navigation-panel"
---

自建导航面板这事儿，以前基本两条路：要么找个服务器跑 Docker（比如 Sun-Panel），要么忍受各种 SaaS 的免费额度限制。

**CF-Navs** 给了第三条路——整个导航站直接跑在 Cloudflare Workers 上，前端、API、D1 数据库、KV 会话全在边缘，**不用任何服务器，免费额度内零成本**。
...
Sun-Panel 老用户搬家最省事——JSON 备份直接导入；没有服务器的，Workers 免费额度完全够用；想要能自定义外观起始页的，22 套主题加自定义 CSS/JS，折腾空间够大。
